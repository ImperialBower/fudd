...
impl TwoCards {
    ...
    pub fn new(first: U32Card, second: U32Card) -> Result<TwoCards, HandError> {
        if first == second {
    ...
    pub fn from_index(index: &'static str) -> Result<TwoCards, HandError> {
        TwoCards::try_from(index)
    }
...
impl From<&[U32Card; 2]> for TwoCards {
...
mod arrays_two_cards_tests {
    ...
    fn is_suited() {
        assert!(
            !TwoCards::new(CardNumber::ACE_CLUBS, CardNumber::ACE_SPADES)
                .unwrap()
                .is_suited()
        );
        assert!(TwoCards::new(CardNumber::ACE_CLUBS, CardNumber::KING_CLUBS)
            .unwrap()
            .is_suited());
    }
